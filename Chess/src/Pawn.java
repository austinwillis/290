...
public class Pawn extends Piece {
	...
	public ArrayList<Move> addmoves(ChessBoard chessBoard) {
		...
		if (row > 1)
			if (chessBoard.spaces[row-2][column].color == 0 && chessBoard.spaces[row-2][column].color == 0 && !this.moved) {
				moves.add(new Move(column, row, column, row-2));
			}
		if (row > 0 && column < 7)
			if (chessBoard.spaces[row-1][column+1].color != 0 && chessBoard.spaces[row-1][column+1].color != this.color)
		...
		if (this.color == 2)
			if (row < 7)
				...
				moves.add(new Move(column, row, column, row+1));
			if (row < 6)
			if (chessBoard.spaces[row+2][column].color == 0 && chessBoard.spaces[row+1][column].color == 0 && !this.moved) {
				moves.add(new Move(column, row, column, row+2));
			}
			if (row < 7 && column < 7)
